...
In addition to these databases, we have provided example input **`raw_reads`** and output files in google drive folder [example_files](https://drive.google.com/drive/u/1/folders/1s4lG2Yq7qXH-iJhCHkvh5BvoADvCeubn) for both viral and bacterial branches of the pipeline. The pipeline can run on a local computer, however, it is really designed to run on HPC clusters due to the computationally intensive nature of BLAST. Depending on the branch of pipeline and the input dataset, the completion of each Snakefile can take from a few hours to several weeks. The viral branch is the fastest, due to having the smallest reference databases. 
